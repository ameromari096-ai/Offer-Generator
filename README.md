...
`webapp/app.py` is a small Flask front-end: a form for the resolved
field values → the same mandatory preview → approve → real DOCX/PDF
generated by the same `offer_agent` engine → download links. It uses
`LocalDevStorageBackend` (local disk, no external credentials needed).
...
- This web app does not parse uploaded CV/hiring-approval documents —
  that extraction is the language-understanding step the chat-based
  `offer-agent` skill performs. The form expects already-resolved values.
